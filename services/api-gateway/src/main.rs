...
async fn health(State(state): State<AppState>) -> Json<HealthResponse> {
    Json(HealthResponse {
        status: "ok",
        service: "alice-medical-saas-gateway",
...
async fn main() {
    ...
    let upstream = std::env::var("UPSTREAM_URL")
        .unwrap_or_else(|_| "http://localhost:8118".to_string());

    let routes: DashMap<String, String> = DashMap::new();
    routes.insert("/api/v1/medical".to_string(), upstream.clone());
    ...
    let port: u16 = std::env::var("PORT")
        .unwrap_or_else(|_| "8218".to_string())
        .parse()
        .unwrap_or(8218);
    let addr = SocketAddr::from(([0, 0, 0, 0], port));
    tracing::info!("alice-medical-saas-gateway listening on {addr}");
